#!/usr/bin/env python3
"""
Enhanced interactive cluster visualization with side panel and uncertainty filtering
"""
...
def load_and_prepare_data():
    ...
    print("="*60)
    print("ENHANCED INTERACTIVE CLUSTER ANALYSIS")
    print("="*60)
    ...
    train_df.loc[train_df['confidence'] < 0.3, 'uncertainty_level'] = 1  # Low uncertainty
    train_df.loc[train_df['confidence'] < 0.1, 'uncertainty_level'] = 2  # High uncertainty
    train_df.loc[~train_df['is_correct'] & (train_df['confidence'] > 0.8), 'uncertainty_level'] = 3  # High conf error
...
def create_enhanced_visualization(train_df):
    """Create enhanced HTML visualization with side panel"""
...
<html>
<head>
    <title>Enhanced Cluster Analysis - Interactive Viewer</title>
    ...
    html_content += f"""
        ...
        function updateVisualization() {{
            ...
            g.selectAll('.point')
                ...
                .enter()
                .append('circle')
                .attr('class', d => `point uncertainty-${{d.uncertainty_level}}`)
        ...
        function getColorScale() {{
            if (currentClustering === 'uncertainty') {{
                ...
                return d => d[currentClustering] === 'Extrovert' ? '#FF6B6B' : '#4ECDC4';
            }} else {{
                const colors = d3.scaleOrdinal(d3.schemeCategory10);
                return d => colors(d['cluster_' + currentClustering]);
            }}
        }}
            ...
            content.innerHTML = `
                ...
                <div class="detail-group">
                    ...
                    <div class="detail-item">
                        ...
                        <span class="detail-value">${{d.confidence.toFixed(3)}}</span>
                    </div>
                    <div class="detail-item">
...
"""
    ...
    # Save HTML
    output_file = OUTPUT_DIR / 'cluster_analysis_enhanced.html'
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write(html_content)
    
    print(f"\nCreated enhanced visualization: {output_file}")
...
def main():
    ...
    create_enhanced_visualization(train_df)
    
    print("\n" + "="*60)
    print("ENHANCED VISUALIZATION COMPLETE")
    print("="*60)
    print("\nFeatures:")
    print("- Click points to see detailed information in side panel")
    print("- Filter by uncertainty level using checkboxes")
    print("- Uncertain points are drawn on top of certain ones")
    print("- Adjust point size and opacity with sliders")
    print("- Switch between PCA, UMAP, and t-SNE views")
    print("- Color by uncertainty, clustering, or personality")
    print("\nOpen cluster_analysis_enhanced.html in a web browser!")
